...
function autoPlaceholder(label) {
  if (!label) return ''
  const normalized = String(label).toLowerCase().trim()
  return normalized ? `Enter ${normalized}` : ''
}
...
export default function Textarea({
  ...
  const resolvedPlaceholder = placeholder === undefined ? autoPlaceholder(label) : placeholder
  ...
  return (
    <label className={clsx('block', className)}>
      ...
      <textarea
        ...
        className={clsx(
          'min-h-[110px] w-full resize-y rounded-2xl border border-black/10 bg-[rgb(var(--panel))]/95 px-3 py-2.5 text-sm text-[rgb(var(--text))] outline-none',
